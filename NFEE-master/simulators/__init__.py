...
def sim_data(gen_params, gen_design, sim_model, n_samples=None, rng=np.random):
    """
    ...
    if n_samples is None:
        ps, ds, xs = sim_data(gen_params, gen_design, sim_model, n_samples=1, rng=rng)
        return ps[0], ds[0], xs[0]
    ...
    while True:
        ...
        ds = gen_design(n_samples, rng=rng)
        xs = sim_model(ps, ds, rng=rng)
    ...
    if n_rem > 0:
        ...
        ps_rem, ds_rem, xs_rem = sim_data(gen_params, gen_design, sim_model, n_rem, rng)
        ps = np.concatenate([ps, ps_rem], axis=0)
        ds = np.concatenate([ds, ds_rem], axis=0)
        xs = np.concatenate([xs, xs_rem], axis=0)

    assert ps.shape[0] == ds.shape[0] == xs.shape[0] == n_samples
...
class RealTimeSimsLoader:
    ...
    def __init__(self, prior, design_prior, model, stats, batch=10**6):
        ...
        self.prior = prior
        self.design_prior = design_prior
    ...
    def load(self, n_samples):
        ...
        return self.ps[:n_samples], self.ds[:n_samples], self.xs[:n_samples]
